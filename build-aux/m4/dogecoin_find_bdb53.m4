...
dnl Copyright (c) 2021 The BrrrFren Core developers
...
AC_DEFUN([BITCOIN_FIND_BDB53],[
  ...
  if test "x$bdbpath" = "xX"; then
    AC_MSG_RESULT([no])
    AC_MSG_ERROR([libdb_cxx headers missing, BrrrFren Core requires this library for wallet functionality (--disable-wallet to disable wallet functionality)])
  ...
  if test "$BDB_LIBS" = ""; then
    ...
    if test "x$BDB_LIBS" = "x"; then
        AC_MSG_ERROR([libdb_cxx missing, BrrrFren Core requires this library for wallet functionality (--disable-wallet to disable wallet functionality)])
    fi
  else
    TLIBS="${LIBS}"
    LIBS="${LIBS} ${BDB_LIBS}"
