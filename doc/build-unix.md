...
You must install required dependencies to build a basic BrrrFren daemon, optional dependencies may vary according to your requirements.
...
BrrrFren Core's released binaries are built using the "depends system", which
...
By default, **BrrrFren Core expects BerkeleyDB 5.3**.
...
Otherwise, you can build BrrrFren Core from self-compiled [depends](/depends/README.md).

**Note**: You only need Berkeley DB if the wallet is enabled (see [*Disable-wallet mode*](#disable-wallet-mode)).

#### Disable-wallet mode
When the intention is to run only a P2P node without a wallet, BrrrFren may be compiled in
...
#### Security
To help make your BrrrFren installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, BrrrFren should be built with a non-executable stack,
...
least 1.5 GB of memory available when compiling BrrrFren Core.
