...
- BrrrFrenMiner : Generates dogecoins (if wallet is enabled).
...
However, in open source software such as PoEM Core, where everyone uses
...
pay attention to for reviewers of PoEM Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: PoEM Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of PoEM Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against PoEM Core so that they can be integrated
