PoEM Core version 0.12.0 is now available from:
...
installer (on Windows) or just copy over /Applications/PoEM-Qt (on Mac) or
...
backwards-compatible with pre-0.10 versions of PoEM Core or other software:
...
pre-0.12 versions of PoEM Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start PoEM Core version 0.11 or
earlier.
...
ECDSA signatures inside PoEM transactions now use validation using
...
Previous versions of PoEM Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of PoEM Core was to raise the default minimum
relay fee.

PoEM Core 0.12 will have a strict maximum size on the mempool. The
...
PoEM Core 0.12 also introduces new default policy limits on the length and
...
PoEM Core 0.12 nodes. PoEM Core will only allow replacement of
...
Note that the wallet in PoEM Core 0.12 does not yet have support for
...
PoEM Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. PoEM Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In PoEM Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in PoEM Core 0.13, and it is at this time undecided whether the
...
PoEM Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
PoEM Core automatically creates a hidden service to listen on, without
manual configuration. PoEM Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if PoEM Core is listening, and
...
PoEMd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals PoEM
Core to use floating fees. By default, PoEM Core will use floating
fees.
...
At all times, PoEM Core will cap fees at `-maxtxfee=<x>` (default:
0.10) BTC.
Furthermore, PoEM Core will never create transactions paying less than
...
- #6214 `87406aa` [OSX] revert renaming of PoEM-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
