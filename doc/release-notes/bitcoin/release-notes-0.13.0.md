PoEM Core version 0.13.0 is now available from:
...
Various code modernizations have been done. The PoEM Core code base has
...
HD wallets are incompatible with older versions of PoEM Core.
...
proposed parameters, a future release of PoEM Core is expected that
...
The two phases are distinct in the PoEM-Qt GUI. During the first one,
...
- #8207 `f7a403b` Add a link to the PoEM-Core repository and website to the About Dialog (MarcoFalke)
...
- #8164 `0f24eaf` [PoEM-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
