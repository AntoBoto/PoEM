BrrrFren Core version 1.14.3 is now available from:
...
BrrrFren Core is extensively tested on Ubuntu Server LTS, Mac OS X and Windows 10.
...
Block download timeouts are expressed as a multiple of block interval, and as such BrrrFren block download times were relatively aggressive, leading to a high number of timeouts. Increased the timeouts to be more flexible to real world conditions.
...
* Return unspents greater or equal than a specific amount in BRRR: minimumAmount (default = 0).
* Return unspents lower or equal than a specific amount in BRRR: maximumAmount (default=unlimited).
* Return unspents with a total number lower or equal than a specific number: maximumCount (default=0=unlimited).
* Return unspents which total is greater or equal than a specific amount in BRRR: minimumSumAmount (default=unlimited).
