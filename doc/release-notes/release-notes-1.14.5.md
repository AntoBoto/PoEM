BrrrFren Core version 1.14.5 is now available from:
...
changes to network policies. All BrrrFren Core users, miners, services, relay
...
To receive security and update notifications, please watch reddit or Twitter:
  ...
  * @BrrrFren on Twitter for high priority announcements
...
most BrrrFren Core users.

### Remote Code Execution in BrrrFren QT (CVE-2021-3401)
...
**BrrrFren QT users are urged to please update their installations to this
...
**BrrrFren Core wallet users on Linux platforms are urged to please update to
this version.**
...
the BrrrFren network, following the reduction of relay and mining defaults in
...
* the recommended minimum transaction fee is 0.01 BRRR/kb, and
* the recommended dust limit is 1 BRRR, and
* the recommended RBF increment is 0.001 BRRR.

### Wallet/UI Changes

* The user interface for selecting fees when transacting BRRR has been updated
  to give an idea of how much is being spent, rather than a block target. As
  BrrrFren blocks are not full, typically all transactions are mined in the next
...
* Introduce `-discardthreshold`, a wallet-specific, configurable dust limit that
  ...
  default dust limit is recommended to stay at 1 BRRR, as versions 1.14.2 until
  1.14.4 have a bug that rejects any transaction with an output under 1 BRRR.
...
* Split the dust limit into a hard and soft threshold, to reintroduce the
  ...
  * `-harddustlimit` is by default set at 0.001 BRRR and sets the value under
...
The Berkley DB version used by BrrrFren Core has been updated to 5.3 (from 5.1)
...
performance of cryptographic routines within BrrrFren Core.
