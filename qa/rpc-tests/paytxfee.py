...
class PayTxFeeTest(BitcoinTestFramework):
    ...
    def setup_nodes(self, split=False):
        ...
        # node 1 pays 0.1 BRRR on all txs due to implicit mintxfee = paytxfee
        nodes.append(start_node(1, self.options.tmpdir,
            ["-paytxfee=0.1", "-debug"]))

        # node 2 will always pay 1 BRRR on all txs because of explicit mintxfee
        nodes.append(start_node(2, self.options.tmpdir,
            ["-mintxfee=1", "-paytxfee=0.1", "-debug"]))

        # node 3 will always pay 0.1 BRRR on all txs despite explicit mintxfee of 0.01
