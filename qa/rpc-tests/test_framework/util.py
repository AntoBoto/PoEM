...
BRRRCOIND_PROC_WAIT_TIMEOUT = 60
...
def initialize_chain(test_dir, num_nodes, cachedir):
    ...
    if create_cache:
        ...
        for i in range(MAX_NODES):
            ...
            args = [ os.getenv("BRRRCOIND", "dogecoind"), "-server", "-keypool=1", "-datadir="+datadir, "-discover=0" ]
            if i > 0:
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None):
    ...
    if binary is None:
        binary = os.getenv("BRRRCOIND", "dogecoind")
...
def stop_node(node, i):
    ...
    return_code = dogecoind_processes[i].wait(timeout=BRRRCOIND_PROC_WAIT_TIMEOUT)
...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s BRRR too low! (Should be %s BRRR)"%(str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 2) * fee_per_kB / 1000:
        raise AssertionError("Fee of %s BRRR too high! (Should be %s BRRR)"%(str(fee), str(target_fee)))
