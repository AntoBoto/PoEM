...
CFeeRate GetBrrrFrenFeeRate(int priority)
{
    switch(priority)
    {
    case SUCH_EXPENSIVE:
        return CFeeRate(COIN / 100 * 521); // 5.21 BRRR, but very carefully avoiding floating point maths
    case MANY_GENEROUS:
...
const std::string GetBrrrFrenPriorityLabel(int priority)
{
    switch(priority)
    {
    case SUCH_EXPENSIVE:
...
CAmount GetBrrrFrenMinRelayFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree)
{
    {
        LOCK(mempool.cs);
    ...
    nMinFee += GetBrrrFrenDustFee(tx.vout, nDustLimit);
...
CAmount GetBrrrFrenDustFee(const std::vector<CTxOut> &vout, const CAmount dustLimit) {
    CAmount nFee = 0;
