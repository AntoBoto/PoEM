    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return PoEMUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, PoEMUnits::separatorNever);
            else
                return PoEMUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != NULL) ? tr("Requested") + " ("+PoEMUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
