// Copyright (c) 2013-2015 The PoEM Core developers
...
void RunTest(const TestVector &test) {
    ...
    BOOST_FOREACH(const TestDerivation &derive, test.vDerive) {
        ...
        // Test private key
        CPoEMExtKey b58key; b58key.SetKey(key);
        BOOST_CHECK(b58key.ToString() == derive.prv);

        CPoEMExtKey b58keyDecodeCheck(derive.prv);
        CExtKey checkKey = b58keyDecodeCheck.GetKey();
        assert(checkKey == key); //ensure a base58 decoded key also matches

        // Test public key
        CPoEMExtPubKey b58pubkey; b58pubkey.SetKey(pubkey);
        BOOST_CHECK(b58pubkey.ToString() == derive.pub);

        CPoEMExtPubKey b58PubkeyDecodeCheck(derive.pub);
