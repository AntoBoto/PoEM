// Copyright (c) 2015-2025 The PoEM Core developers
...
BOOST_FIXTURE_TEST_SUITE(poem_tests, TestingSetup)
...
BOOST_AUTO_TEST_CASE(subsidy_first_100k_test)
{
    ...
    for (int nHeight = 0; nHeight <= 100000; nHeight++) {
        ...
        CAmount nSubsidy = GetPoEMBlockSubsidy(nHeight, params, ArithToUint256(prevHash));
...
BOOST_AUTO_TEST_CASE(subsidy_post_145k_test)
{
    ...
    for (int nHeight = 145000; nHeight < 600000; nHeight++) {
        const Consensus::Params& params = mainParams.GetConsensus(nHeight);
        CAmount nSubsidy = GetPoEMBlockSubsidy(nHeight, params, prevHash);
    ...
    CAmount nConstantSubsidy = GetPoEMBlockSubsidy(600000, mainParams.GetConsensus(600000), prevHash);
    BOOST_CHECK_EQUAL(nConstantSubsidy, 10000 * COIN);

    nConstantSubsidy = GetPoEMBlockSubsidy(700000, mainParams.GetConsensus(700000), prevHash);
    BOOST_CHECK_EQUAL(nConstantSubsidy, 10000 * COIN);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_difficulty_limit)
{
    ...
    BOOST_CHECK_EQUAL(CalculatePoEMNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1e00ffff);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_pre_digishield)
{
    ...
    BOOST_CHECK_EQUAL(CalculatePoEMNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1c15ea59);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield)
{
    ...
    BOOST_CHECK_EQUAL(CalculatePoEMNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b671062);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield_modulated_upper)
{
    ...
    BOOST_CHECK_EQUAL(CalculatePoEMNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b4e56b3);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield_modulated_lower)
{
    ...
    BOOST_CHECK_EQUAL(CalculatePoEMNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b335358);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield_rounding)
{
    ...
    BOOST_CHECK_EQUAL(CalculatePoEMNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b6558a4);
}
