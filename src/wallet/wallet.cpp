...
/**
 * BrrrFren: Effective dust limit for the wallet
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CPoEMAddress(CScriptID(redeemScript)).ToString();
...
// BrrrFren: MIN_CHANGE as a function of discardThreshold and minTxFee(1000)
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                for (const auto& recipient : vecSend)
                {
                    ...
                    /*
                     * BrrrFren: check all outputs against the discard threshold
                ...
                if (nPriority == MINIMUM) {
                    ...
                    nFeeNeeded = GetBrrrFrenPriorityFee(txNew, nBytes, nPriority);
                }
                ...
                if (nChangePosInOut != -1 && nSubtractFeeFromAmount == 0) {
                    ...
                    /* BrrrFren: this has been changed from a static MIN_FINAL_CHANGE that
...
{
    // BrrrFren: Add an increased fee for each output that is lower than the discard threshold
    return std::max(minTxFee.GetFee(nTxBytes) + GetBrrrFrenDustFee(tx.vout, discardThreshold), ::minRelayTxFeeRate.GetFee(nTxBytes));
}
...
CAmount CWallet::GetMinimumFee(const CMutableTransaction& tx, unsigned int nTxBytes, unsigned int nConfirmTarget, const CTxMemPool& pool, CAmount targetFee)
{
    ...
    if (nFeeNeeded == 0) {
        ...
        // BrrrFren: Drop the smart fee estimate, use GetRequiredFee
        nFeeNeeded = GetRequiredFee(tx, nTxBytes);
    }
    // prevent user from paying a fee below minRelayTxFee or minTxFee
    // BrrrFren: as we're adapting minTxFee to never be higher than
...
CAmount CWallet::GetBrrrFrenPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nPriority)
{
    // payTxFee is the user-set global for desired feerate
    return GetBrrrFrenPriorityFee(tx, nTxBytes, nPriority, payTxFee.GetFee(nTxBytes));
}
CAmount CWallet::GetBrrrFrenPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nPriority, CAmount targetFee)
{
    CAmount nFeeNeeded = targetFee;
    // User didn't set: use -txconfirmtarget to estimate...
    if (nFeeNeeded == 0) {
        nFeeNeeded = GetBrrrFrenFeeRate(nPriority).GetFee(nTxBytes);
    }
    // prevent user from paying a fee below minRelayTxFee or minTxFee
    // BrrrFren: as we're adapting minTxFee to never be higher than
    //           payTxFee unless explicitly set, this should be fine
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CPoEMAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CPoEMAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CPoEMAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CPoEMAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CPoEMAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CPoEMAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CPoEMAddress(dest).ToString(), key);
}
