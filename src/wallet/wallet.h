...
/* 1.14.5: set the wallet's discard threshold to 1 BRRR because that's what 97%
 ...
 */
/* 1.14.6: set the wallet's discard threshold to 0.01 BRRR. Very network
...
/*
 * BrrrFren: Scaled to 1/10th of the recommended transaction fee to make RBF
...
/*
 * BrrrFren: Creating change outputs at exactly the dustlimit is counter-
...
public:
    ...
    /**
     * BrrrFren: Get a fee targeting a specific transaction speed.
     */
    CAmount GetBrrrFrenPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed);
    /**
     * BrrrFren: Get a fee targeting a specific transaction speed.
     */
    static CAmount GetBrrrFrenPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed, CAmount targetFee);
    /**
    ...
    /**
     * Return the minimum required fee taking into account the
     * floating relay fee and user set minimum transaction fee, but not the BrrrFren dust fee.
     */
